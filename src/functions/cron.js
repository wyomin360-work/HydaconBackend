...
function initCronJobs() {
  ...
  cron.schedule("5 0 * * *", async () => {
    try {
      ...
      const activeSeason = await LoyaltySeason.findOne({ active: true, isArchived: { $ne: true } });
      if (activeSeason && activeSeason.endDate < now) {
        console.log(`Season ${activeSeason.name} ended. Processing rollover...`);
        ...
        if (nextSeason) {
          ...
          const allProgress = await UserTierProgress.find({ seasonId: activeSeason._id });

          for (const progress of allProgress) {
            let carryForwardPoints = 0;
            if (activeSeason.carryForwardBehavior === CARRY_FORWARD_BEHAVIOR.FULL) {
              carryForwardPoints = progress.qualificationPoints;
            } else if (activeSeason.carryForwardBehavior === CARRY_FORWARD_BEHAVIOR.PERCENTAGE && activeSeason.carryForwardPercentage) {
              carryForwardPoints = Math.floor(progress.qualificationPoints * (activeSeason.carryForwardPercentage / 100));
            }
            ...
            await UserTierProgress.findOneAndUpdate(
              { userId: progress.userId, seasonId: nextSeason._id },
              {
                $setOnInsert: {
                  ...
                  currentTierId: progress.currentTierId, // Retain tier initially, might downgrade later
                  qualificationPoints: carryForwardPoints,
                }
              },
              { upsert: true, new: true, setDefaultsOnInsert: true }
            );

            // Here we would ideally evaluate downgrade policy based on new tier thresholds
          }
        } else {
  ...
  cron.schedule("10 0 * * *", async () => {
    try {
      ...
      if (expiredTransactions.length > 0) {
        console.log(`Found ${expiredTransactions.length} expired point transactions to process.`);
