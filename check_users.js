...
async function run() {
  ...
  const recentUsers = await User.find({}).sort({ updatedAt: -1 }).limit(5).populate("roleId");
  ...
  for (const u of recentUsers) {
    ...
    Object.keys(content).forEach(placement => {
      content[placement].forEach(item => {
        if (item.type === 'POPUP') {
          foundAny = true;
          console.log(`  - [${placement}] "${item.title}" (popupType: ${item.popupType}, rule: ${item.ruleSetId ? (item.ruleSetId.name || item.ruleSetId) : "None"})`);
        }
      });
    });
