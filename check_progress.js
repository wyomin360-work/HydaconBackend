...
async function run() {
  ...
  const progress = await UserTierProgress.find({ userId: "6a0ca8341e21fd8cb5d1f2c1" }).populate("currentTierId");
  ...
  progress.forEach(p => {
    ...
    console.log(`Tier Name: ${p.currentTierId ? p.currentTierId.name : "None"}`);
