...
async function run() {
  ...
  console.log("Found gifts with user in rewardedUsers:", gifts.map(g => g.name));

  for (const gift of gifts) {
      console.log(`\nTesting Gift: ${gift.name}`);
      const details = await giftService.getGiftDetails(userId, gift._id);
      console.log("getGiftDetails isFreeClaimable:", details.data.isFreeClaimable);

      const eligibility = await giftService.getGiftEligibility(userId, gift._id);
      console.log("getGiftEligibility isRewardedUser:", eligibility.data.isRewardedUser);
  }
